...
public:
    SerialPort();
    // 串口初始化
    SerialPort(const char* filename, int buadrate);		// 参数：串口名称   波特率
    // 重启串口
    void restart_serial(void);
    // 发送数据
    void send_data(const struct serial_transmit_data& data);
    // 接收数据
    bool read_data(const struct serial_receive_data* data, bool& mode, float& bulletspeed, float& buff_offset_x, float& buff_offset_y, int& col);
    // 关闭串口
    void close_port();
